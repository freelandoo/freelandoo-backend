...
const log = createLogger("r2.uploadCasaParticipantMedia");
...
// kind: avatar | cover | product
module.exports = async function uploadCasaParticipantMediaToR2({ file, kind = "media" }) {
  log.info("upload.start", { kind, mimetype: file?.mimetype });
  const fileExt = getFileExt(file.originalname);
  const fileName = `casa-participants/${kind}-${crypto.randomUUID()}.${fileExt}`;
