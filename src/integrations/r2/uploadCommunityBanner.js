...
const log = createLogger("r2.uploadCommunityBanner");
...
module.exports = async function uploadCommunityBannerToR2({ id_profile, file }) {
  log.info("upload.start", { id_profile, mimetype: file?.mimetype });
  const fileExt = getFileExt(file.originalname);
  const fileName = `community-banners/${id_profile}-${crypto.randomUUID()}.${fileExt}`;
