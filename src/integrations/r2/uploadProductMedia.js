...
const log = createLogger("r2.uploadProductMedia");
...
function buildKey(id_profile, id_profile_product, originalname, suffix = "") {
  const fileExt = getFileExt(originalname);
  const base = safeName(originalname || "").replace(/\.[^.]+$/, "") || "media";
  const tag = suffix ? `-${suffix}` : "";
  return `shop-products/${id_profile}/${id_profile_product}/${base}${tag}-${crypto.randomUUID()}.${fileExt}`;
}
...
module.exports = async function uploadProductMediaToR2({ id_profile, id_profile_product, file }) {
  ...
  const key = buildKey(id_profile, id_profile_product, file.originalname);
