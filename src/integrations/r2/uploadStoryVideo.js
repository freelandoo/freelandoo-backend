// src/integrations/r2/uploadStoryVideo.js
const { PutObjectCommand } = require("@aws-sdk/client-s3");
const crypto = require("crypto");
const r2 = require("../../services/r2Client");
const { createLogger } = require("../../utils/logger");

const log = createLogger("r2.uploadStoryVideo");
...
function buildKey(id_profile, originalname, suffix = "") {
  const fileExt = getFileExt(originalname);
  const original = safeName(originalname || "");
  const base = original ? original.replace(/\.[^.]+$/, "") : "story";
  const tag = suffix ? `-${suffix}` : "";
  return `stories/${id_profile}/${base}${tag}-${crypto.randomUUID()}.${fileExt}`;
}
...
module.exports = async function uploadStoryVideoToR2({ id_profile, file }) {
  ...
  const fileName = buildKey(id_profile, file.originalname);
