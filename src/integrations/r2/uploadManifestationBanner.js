...
const log = createLogger("r2.uploadManifestationBanner");
...
module.exports = async function uploadManifestationBannerToR2({ file, kind = "banner" }) {
  log.info("upload.start", { kind, mimetype: file?.mimetype });
  const fileExt = getFileExt(file.originalname);
  const fileName = `manifestation-banners/${kind}-${crypto.randomUUID()}.${fileExt}`;
